...
function lda_correct = ImgCls_LDA_kNN(Train, Test, k, d)
    ...
    for i = 1:size(Test,2)
        ...
        [~, lda_sort_ind] = sort(ldanorms);
        lda_count_ind = lda_sort_ind(1:k);
        lda_count_cls = ceil(lda_count_ind / 5);
        lda_testpred = [lda_testpred, mode(lda_count_cls)];
    end
